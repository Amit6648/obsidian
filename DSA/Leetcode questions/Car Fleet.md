...
- The reason is cause if the current element that is behind the previous element can't catchup with previous that means the elements that are behind the current are even behind the current element that means before reaching the previous element they w
