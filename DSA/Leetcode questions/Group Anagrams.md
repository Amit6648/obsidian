...
- So if we treat each string as  a element we can basically say that we have to find elements that are same. so rather than comparing elements normal way we can use a [[Unordered Hash maps]] we can use a element as a key and store all the elements in the value that matc
