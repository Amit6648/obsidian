...
- So we can basically use [[Unordered Hash maps]] to store elements and their indexes as we will add a element we will check if we have seen this element's left over part or **Target - current element** this approach is similar to [[Contains Duplicate]] where we find if a e 
